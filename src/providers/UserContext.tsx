...
interface IUserContext {
  user: IUserLoginReturn | undefined;
  userLogin: (formData: TLoginFormData) => Promise<void>;
  userRegister: (formData: TRegisterFormData) => Promise<void>;
  userLogout: () => void;
}
...
export const UserProvider = ({ children }: IUserProviderProps) => {
  const [user, setUser] = useState<IUserLoginReturn>();
  const navigate = useNavigate();

  const userLogin = async (formData: TLoginFormData) => {
    try {
  ...
  const userRegister = async (formData: TRegisterFormData) => {
    try {
